...
- **本质需要并行** → 携带**病因清单**进入 [07_parallel_splitting](../07_parallel_splitting/SKILL.md) 全流程（分析 → 实施 → 验证）：**崩溃位置（Step 0 取证的 traceback 帧与算子名）**、OOM 发生阶段（加载 / 哪个 forward 阶段）、超限张量类别（权重 / 持久状态 / 激活）、缩放规律——07 的「第零步病因对账」以此校验切分方案是否覆盖病灶，禁止只治速度不治容量；切分验证通过后回归 0.5 完成适配精度验证（天然在多卡配置下执行）
