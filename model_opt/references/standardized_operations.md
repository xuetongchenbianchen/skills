...
1. **强制检查**：每次运行代码（benchmark / profiling / 精度验证 / 功能测试）前，必须先用 `npu-smi info` 检查各卡上的进程占用
2. **确认归属**：发现进程后，先用 `ps -fp <PID>` 确认进程身份；仅清理确认与当前任务无关的进程，疑似他人任务时必须先向用户确认
3. **清理后复查**：`kill <PID>`（顽固进程用 `kill -9`）后再次 `npu-smi info`，确认显存/算力已释放，再启动运行
4. **理由**：无关进程争抢算力/显存会导致性能数据失真（utilization、端到端时间被污染）或 OOM，基线数据一旦污染不可复用
