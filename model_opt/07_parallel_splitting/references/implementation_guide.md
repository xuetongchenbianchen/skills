...
> **切分只验切分（耗时边界）**：本节验证回答且仅回答"切分实施是否正确"——张量分布是否符合约定表、通信原语是否配对、浮点容差是否在档。**不承担全量精度回归**（多样本、多 shape、top-1 一致性等）——那是切分通过、回归 Phase 0「0.5 精度验证」（golden 对齐）的职责；Phase 4 门禁服务其后的优化循环，与切分验证无关。样本纪律（唯一定义处）：固定 **1 条代表性真实样本 + 1 条最小冒烟输入**，代表性样本覆盖被切维度的敏感特征（切 batch/含 padding → padding 边界；切 seq → 生产代表性长度；切权重 → 任一常规样本），规模取生产主流 regime 的中位代表（仅当生产负载本身即最大规模时才取最大）——切分 bug 靠维覆盖暴露，不靠样本数量。
