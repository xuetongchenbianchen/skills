...
★ 实现的模型与 analysis_workflow.md 第三步一致：
  - 显存: M = M_param/p_param + M_state/p_state + M_act/p_act + M_overhead，阈值 HBM × 0.8
...
states 尺寸由 agent 从源码提取；hardware 可省略用典型值）:
  {
    ...
    "candidates": [{"tp": 2, "pp": 1, "sp": 1, "cp": 1, "ep": 1, "dp": 1}]
  }

注意: 估算为量级判断（整体误差 10-30%），用于候选间相对比较与剪枝；
绝对耗时以回归 Phase 1 后的实测为准。跨维重分布（row↔col）依赖具体模型
结构，不在枚举/估算范围内，需要时在报告基础上手工追加。
"""
...
TYPICAL_HW = {"alpha_us": 20.0, "bandwidth_GBs": 392.0}  # 昇腾 910B 典型值，仅量级估算
...
def load_spec(path):
    ...
    字段定义（✅ = 必填；右列 = 默认值；字节数除注明外）:
      ...
      hardware.alpha_us                      20    通信启动延迟 µs（典型值，可覆盖）
      hardware.bandwidth_GBs                 392   单链路带宽 GB/s（典型值，可覆盖）
      candidates                             —     显式候选列表；缺省时必须给 --cards
      candidates[].tp/pp/sp/cp/ep/dp         1     各维度切分并行度（tp 建议 2 的幂）
    ...
    allowed_top = {"dims", "bytes", "states", "hardware", "candidates"}
    ...
    allowed_nested = {
        ...
        "states": {"param", "state", "act", "overhead"},
        "bytes": {"param", "act"},
    ...
    d = {
        ...
        "param_bytes": states["param"], "state_bytes": states.get("state", 0.0),
...
def enumerate_candidates(cards, has_moe):
    """枚举 tp×pp×cp×ep 整除 cards 的组合（tp 取 2 的幂，dp 补余量，sp 固定 1）。
    ...
    for tp in tp_options:
        for pp in divisors(cards // tp):
            for cp in divisors(cards // tp // pp):
                ...
                for ep in eps:
                    dp = cards // (tp * pp * cp * ep)
                    out.append({"tp": tp, "pp": pp, "sp": 1, "cp": cp, "ep": ep, "dp": dp})
    return out
...
def main():
    ...
    cands = [dict({"tp": 1, "pp": 1, "sp": 1, "cp": 1, "ep": 1, "dp": 1}, **c)
             for c in spec.get("candidates", [])]
    if args.cards:
    ...
    if not feasible:
        ...
        return
    print("可行候选按通信耗时排序（估算为相对比较，绝对值以实测为准）:")
    for rank, (r, i) in enumerate(sorted(feasible, key=lambda x: x[0]["comm_total"]), 1):
        c = r["cand"]
        tag = " ".join(f"{k}={v}" for k, v in c.items() if v > 1) or "单卡"
        print(f"  {rank}. [候选 {i}] {tag}: 显存 {fmt_gb(r['m_total'])}, "
              f"通信 {r['comm_total'] * 1e3:.3f}ms/步")
