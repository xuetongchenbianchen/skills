...
**Level 1（快速验证）** 的逻辑是输出对比——比较 baseline 和 optimized 的输出，不需要 ground truth：

```
模型输出 Y → 提取可观测量 Φ(Y) → 计算距离 D → 判定
```

其中 Φ（可观测量）是从模型输出中提取的、能反映任务成败的某个性质；D 是量化 Φ 变化大小的距离函数。

关键在于选对 Φ——它必须捕捉"决定任务成败的那个性质"。选错 Φ 会导致误判。
...
**优化路径影响 Level 1 的 Φ 选择方向**：

- **数值等价路径**（没改算法，只改实现）：Φ(Y) = Y 本身，D = 逐值误差预算。期望输出在浮点误差内一致。
- **近似算法路径**（改了算法、精度、步数等）：Φ(Y) = 任务代理量，D = 语义距离。输出一定不同，问题是变化是否可控。
