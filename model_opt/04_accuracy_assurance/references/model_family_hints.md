...
> 速查表见主文件 SKILL.md「三、确定比什么」。本文件提供各模型类型的详细推理过程，供遇到边界情况或不在速查表中的模型时参考。文中「类别 N」= 主文件「四、选择怎么比」的五类输出编号，度量定义以主文件为准。
...
**度量**：logits → 类别 1；贪心 token 序列 → 完全匹配率（argmax 对 logits 微小 diff 敏感，可能翻转）；采样解码 → benchmark（MMLU / HumanEval / BLEU / Rouge）
...
**度量**：分类 logits → 类别 1；feature map → 类别 3（cosine + max_abs 可作快速比对）；检测 → 类别 4（mAP / mIoU，比后处理后结果）
...
**度量**：LLM logits → 类别 1；图片 → 类别 5；概率分布 → 类别 1（softmax 后 JS/KL）
...
**度量**：embedding → 类别 2；物理量 → 类别 3（力的绝对值影响轨迹，不能只看方向）
...
**度量**：均属类别 3——energy 用相对误差，forces/stress 以 max_abs 为主

### 蛋白质结构预测

**直接输出**：3D 坐标
**验证边界**：下游功能指标（坐标 diff 不代表结构等价）
**度量**：类别 4——RMSD（整体偏差）、TM-score（结构相似度，比 RMSD 更语义化）

### 天气/气候预测

**直接输出**：物理场（温度、湿度、风速等网格数据）
**验证边界**：下游功能指标（逐点 diff 不代表预报质量）
**度量**：类别 3——空间 RMSE、异常相关系数（ACC）

### 时序预测

**直接输出**：预测序列
**验证边界**：直接输出（确定性预测）或下游功能指标（多步滚动有累积误差）
**度量**：类别 3（MAE / MSE / MAPE）；多步滚动预测后半段是否漂移单独检查
